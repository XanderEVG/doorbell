...
void SetTask(TPTR TS)
{
  ...
  if (SREG & (1<<SREG_I))  // Если прерывания разрешены, то запрещаем их.
  {
    cli();
  ...
  {                                   // с значением Idle - конец очереди.
    ...
    {
      if (nointerrupted)  sei();    // Если мы не в прерывании, то разрешаем прерывания
  ...
  if (nointerrupted) sei();       // И включаем прерывания если не в обработчике прерывания.
}
...
void SetTimerTask(TPTR TS, u16 NewTime)
{
  ...
  if (SREG & (1<<SREG_I))       // Проверка запрета прерывания, аналогично функции выше
  {
    cli();
    nointerrupted = 1;
  }
  ...
  for(index=0; index!=MainTimerQueueSize+1; ++index)  //Прочесываем очередь таймеров
  {
    if(MainTimer[index].GoToTask == TS)       // Если уже есть запись с таким адресом
    {
      ...
      if (nointerrupted)  sei();            // Разрешаем прерывания если не были запрещены.
